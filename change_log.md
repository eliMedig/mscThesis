...
## v1.58
- Updated the default and example portfolio datasets

## v1.57
- Replaced random portfolio generation with the bundled default portfolio.csv
- Anomaly detection now seeds the standard portfolio into the data dir on first run

## v1.56
- Condensed code comments to one-line rationale and removed em-dashes
- Removed dead code: Agent.complete, Agent.is_ready, coaching_agent.respond
- Default docker-compose.yml now runs out of the box; home-lab config split to docker-compose.homelab.yml
- Rewrote README with two tested run paths

## v1.55
- Added pending Tacit Externalisation count to the sidebar menu

## v1.54
- Added deterministic governance tasks for every anomaly flag
- Added owner-grouped task review with reassignment and status controls
- Added pending task count and confirmation-protected task cleanup

## v1.53
- Added persistent per-response internal process traces with ordered orchestration events and evidence
- Replaced standalone domain analysis with an Internal process steps expander on every coaching response
- Added legacy-history fallback and explicit trace labels for failures, skipped actions, and reflection scheduling

## v1.52
- Reflection patch format now uses level-4 headings, a shorter Rationale section, and tighter guidance
- Removed dash-style punctuation from the agent system prompts and reflection prompts
- Bumped prompt seed and reflection output versions so existing installs refresh the new prompts

## v1.51
- Updated reflection patches to use compact headings with an importance indicator, insight, tacit-knowledge rationale, and suggested prompt addition at the bottom
- Added a reflection-output schema version so deployed settings refresh the shared output instruction without resetting agent prompts

## v1.50
- Reduced the vertical gap between Data Sources and the Conversation section to give the chat more usable height

## v1.49
- Anomaly detection status and empty-state hint are now plain lines (no box-inside-the-expander)

## v1.48
- Prompt Engineering note is now a plain line (no box-inside-the-expander)
- Sidebar shows the framework name and version on separate lines
- Added empty exampleFiles/ folder for tester sample documents

## v1.47
- Sidebar now reads "Tacit Externalisation Agent Framework | TEAF"
- Replaced the Prompt Engineering channel blurb with a one-line info note

## v1.46
- Both "no model assigned" warnings now use the info icon and render as a red box with white text in light and dark mode

## v1.45
- Removed unused embedding config (OpenAI backend constants); kept the local model actually used
- Removed dead, never-called functions: anomaly.regenerate, rag.embed, and store get_session/get_patch/add_agent_rag/delete_agent_rag/list_user_editable_collections
- No behaviour change — audit confirmed zero call sites; 30 tests still pass

## v1.44
- Reflection-prompt editor now reflects the new defaults: the version-gated refresh clears stored reflection-prompt overrides that were shadowing the code defaults

## v1.43
- Rewrote both reflection prompts: coaching-knowledge self-reflection and domain-steering reflection, with a hard integrity test
- Reflection now sees the target agent's CURRENT prompt, so it only proposes genuinely new additions (not restatements)
- Kept the NO_CHANGE sentinel + 3-section patch format; domain pass still patches the Domain Agent prompt

## v1.42
- Rewrote the Coaching Agent system prompt (modes, GROW arc, questioning/deepening, AI self-disclosure)
- Added non-negotiable coaching boundary + escalation rules: refuse out-of-scope/distress topics, hand off to a human
- Rewrote the Domain Agent system prompt around its three sources (documents RAG + portfolio SQL + hybrid anomaly) with a FINDINGS/GROUNDING/CONFIDENCE&GAPS output contract
- Kept structured-output delegation (no <<DOMAIN_QUERY>> syntax) and mode-as-badge, matching the built tool
- Version-gated prompt refresh: existing installs adopt the new seed prompts once (conversations/patches/models untouched)
