"""Admin — model + API-key registry.

Register a model as (name, provider, model_string, api_key), then assign it to an
agent on the Agents page. Models are editable here so you can swap the model behind
an agent on the fly (useful when evaluating which model fits the work). Keys may
also come from poc/.env; keys entered here are stored in the gitignored SQLite DB
...
def _test_model(m) -> None:
    """Connectivity / credentials check: one real minimal call through the wrapper."""
