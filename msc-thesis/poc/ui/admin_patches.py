"""Admin — prompt-patch review (TEAF: human-in-the-loop knowledge governance).

Self-reflection externalises a structured prompt patch (§9). A human reviews each
patch and Approves or Rejects it — reversibly. On approve, the "Suggested
system-prompt addition" is appended to the target agent's system prompt; on reject
it is removed again. Patches are NEVER auto-applied.

Three tabs (Pending / Approved / Rejected); a decision can always be changed.
"""
import threading
import time
...
# Background consolidation so BOTH agents can run at once: each click starts a
# daemon thread keyed by agent_id; a running line shows per active job until done.
