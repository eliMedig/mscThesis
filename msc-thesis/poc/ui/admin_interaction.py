"""Admin — interaction trigger configuration.

Two distinct orchestration triggers (§7). They are presented A → B, and only ONE
is configurable: Trigger A is a per-turn judgement (deliberately not a knob);
Trigger B (self-reflection) is turn-driven and configurable.
"""
import html
...
def _flow_diagram_html(every_n: int, at_end: bool) -> str:
    """Fixed SVG diagram so labels stay attached to the intended arrows.

    Generously spaced: wide canvas, ~110px gaps between the top-row nodes and long
    edges so every arrow label (incl. "if approved") sits clear of the boxes. On
    narrow viewports the diagram keeps a legible min-width and scrolls horizontally."""
