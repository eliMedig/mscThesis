...
def test_generate_portfolio_shape(tmp_env):
    path = anomaly.generate_portfolio(n=200, seed=42)
    assert path.exists()
    import csv
    ...
    required = {"app_id", "owner", "lifecycle_state", "business_criticality",
                "technical_maturity", "last_reviewed_date", "compliance_status",
                "hosting", "annual_cost_chf", "vendor", "category"}
