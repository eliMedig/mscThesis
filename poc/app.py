...
from ui import admin_patches, chat, settings, tasks
...
_STYLE = """
  ...
  .teaf-chat-section-spacer { height:.45rem; margin:0; padding:0; }
  [data-testid="stMarkdownContainer"]:has(.teaf-chat-section-spacer) {
    margin:0 !important; padding:0 !important;
  }
  h3.teaf-conversation-heading { color:var(--text) !important; margin:0 0 .35rem 0; }
  .teaf-session-tag {
  ...
  /* Starter chips render in normal flow so they share the chat input's bounds without
  ...
  /* Descendant selectors, not `> button`: a button with help= is wrapped in a tooltip
     target, so a direct-child selector misses it and it renders dark in light mode. */
  .stButton button,
  ...
  /* Settings panel: one rule styles every button uniformly in both themes,
     regardless of primary, secondary, or help-wrapped. */
  ...
  /* The !important backgrounds above suppress Streamlit's own :hover, so define it here. */
  ...
  /* "No model assigned" warnings: red box with white text in both themes, scoped to the
     keyed container so they override the neutral alert styling above. */
  .st-key-model_warnings [data-testid="stAlert"],
  .st-key-model_warnings [data-testid="stAlertContainer"] {
    background:#b91c1c !important; border:1px solid #b91c1c !important;
  }
  .st-key-model_warnings [data-testid="stAlert"] *,
  .st-key-model_warnings [data-testid="stAlertContainer"] * {
...
pending_tasks = store.count_governance_tasks("pending")
tasks_title = f"Tasks ({pending_tasks})" if pending_tasks else "Tasks"
pending_externalisations = store.count_patches("pending")
tacit_title = (
    f"Tacit Externalisation ({pending_externalisations})"
    if pending_externalisations else "Tacit Externalisation"
)

nav = st.navigation([
    st.Page(chat.render, title="Chat", icon="💬", url_path="chat", default=True),
    st.Page(tasks.render, title=tasks_title, icon="📋", url_path="tasks"),
    st.Page(admin_patches.render, title=tacit_title, icon="📝", url_path="tacit"),
    st.Page(settings.render, title="Settings", icon="⚙️", url_path="settings"),
])

st.sidebar.markdown(
    f"<div class='teaf-version'>Tacit Externalisation Agent Framework | TEAF<br>v{config.APP_VERSION}</div>",
