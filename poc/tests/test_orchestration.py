"""Tests for the orchestration loop — the judgment-driven domain-call branch (§7).

Uses fake agents so no API key / network is required: the point is to verify the
orchestration wiring (decide → domain → finalize, persistence, mode tagging),
not the LLM.
"""
import config
